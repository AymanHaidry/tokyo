...
def reset_last_results(monkeypatch):
    """Reset LAST_RESULTS before every test to avoid cross-test pollution."""
    ts.LAST_RESULTS.clear()
...
def test_main_runs_flow_7_with_data(monkeypatch, capsys, tmp_path):
    """Option 7 with LAST_RESULTS populated should generate a bug report."""
    monkeypatch.chdir(tmp_path)
    ts.LAST_RESULTS.update({"python_version": True})
    monkeypatch.setattr("troubleshooter.ask", lambda *a, **k: "7")
    monkeypatch.setattr("builtins.input", lambda *a, **k: "")
    main()
    captured = capsys.readouterr()
    assert "Bug report saved" in captured.out or "No previous scan data" not in captured.out
