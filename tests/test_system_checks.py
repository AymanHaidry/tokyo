...
# --- New pygame/audio tests ---

def test_check_pygame_returns_bool():
    """check_pygame returns True if pygame mixer works, False otherwise."""
    result = check_pygame()
    assert isinstance(result, bool)

def test_check_audio_player_returns_list():
    """check_audio_player returns a list of available backends."""
    result = check_audio_player()
    assert isinstance(result, list)
    # Each item should be a string (player name)
